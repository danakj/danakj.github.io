...
attacks are going on all the time, with no real way to actually know about it
until you find one being used. The ones that go undetected can remain in active
use indefinitely, until the software vendor makes systemic changes to make
those types of bugs go away.

Software vendors never talk about *how* a 0-day bug is being used, so news
media doesn't have much to go on and could only really speculate. However
these bugs are worth [millions of dollars](
...
supporting [human rights abuses](https://thehackernews.com/2021/08/bahraini-activists-targeted-using-new.html).
...
expecting integers to overflow, and wrap around. So when they do, the program gets into an
unexpected state, and can become a useful
[weird machine](https://en.wikipedia.org/wiki/Weird_machine) for an attacker.

Every major C or C++ codebase that is ubuiquitous across user devices will be similarly valuable to
attackers, and vulnerable to the same classes of bugs. And we see active 0-day bugs being reported in those software products too (such as
...
conference had multiple talks on C++ Successor Languages, naming moving off C++ as the
strategy to stop putting customers and users into harm's way.
...
- Cpp2: [The Evolution of C++ - A Typescript for C++ - Herb Sutte](https://www.youtube.com/watch?v=fJvPBHErF2U)
...
*integer overflow*, for which C++ lacks the guardrails to protect against for developers
...
Rust is famous for being memory safe yet only traps on integer overflow in debug builds,
...
include bounds checks. There is no `memcpy`, `memset`, `malloc`, or `alloca` functions that
...
vulnerability depending on how code ends up being ported to or written in Carbon.
...
working with APIs build on primitive types. And it allows the migration of APIs to use them
even while callers continue to use them with primitive types. However, they will catch at
compile time any users that were losing data from their primitive types.

These types protect against integer overflow by panicing in the default operations (`+`, `-`,
`*`, `/`, `%`). But since they are a user-defined type, they can providing a rich API for handling
...
enough APIs!
So rather than make something completely new, the numeric APIs are modelled after Rust's numeric
APIs. If you are familiar with Rust, then you already know the provided APIs, there's nothing
...
and floating point types are all well-defined, and invalid type conversions are still prevented
at compile time.
...
While a piece of software that is exposes you to attackers on the internet (like a browser,
...
on these types, and their associated types in the library, but they are already proving useful.
